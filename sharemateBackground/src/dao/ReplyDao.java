...
public class ReplyDao {
	...
	public List<ReplyBean> getRepliesBycommentId(int commentId){
		...
		Connection conn=DataBase.getConnection();
		PreparedStatement pstmt=null;
		String sql="select reply_id,user_id,reply_detail,reply_time,reply_like_count from reply where comment_id=? order by reply_time desc";
		try {
			pstmt=conn.prepareStatement(sql);
	...
	public void addCommentReply(ReplyBean reply) {
		Connection conn=DataBase.getConnection();
		PreparedStatement pstmt=null;
		String sql="insert into reply(comment_id,user_id,reply_detail,reply_time,reply_like_count) values(?,?,?,NOW(),0)";
		try {
			pstmt=conn.prepareStatement(sql);
	...
	public void addReReply(ReplyBean reply) {
		Connection conn=DataBase.getConnection();
		PreparedStatement pstmt=null;
		String sql="insert into reply(re_reply_id,user_id,reply_detail,reply_time,reply_like_count) values(?,?,?,NOW(),0)";
		try {
			pstmt=conn.prepareStatement(sql);
